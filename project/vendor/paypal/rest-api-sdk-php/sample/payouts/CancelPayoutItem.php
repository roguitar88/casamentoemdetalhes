...
$payoutItems = $payoutBatch->getItems();     // Uncaught Error: Call to a member function getItems() on boolean
...
try {
    if ($payoutItem->getTransactionStatus() == 'UNCLAIMED') {
    	...
    	echo "Unclaimed Payout Item Canceled Successfully";
	} else {
		...
		echo "Unclaimed Payout Item Not Canceled. Payout Item Status is not UNCLAIMED.";
    }
} catch (Exception $ex) {
    // NOTE: PLEASE DO NOT USE RESULTPRINTER CLASS IN YOUR ORIGINAL CODE. FOR SAMPLE ONLY
